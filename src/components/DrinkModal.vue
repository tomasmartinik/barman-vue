...
import {  globalIngredients, selectIngredient, globalSelectedIngredients } from "../api/api";
...
export default {
  ...
  computed: {
    filteredAlcoholicDrinks() {
      return globalIngredients.data["alco ingredients"].filter((d) => d.includes(this.search));
    },
    filteredNonAlcoholicDrinks() {
      return globalIngredients.data["non alco ingredients"].filter((d) => d.includes(this.search));
    },
  },
  methods: {
